...
def generate_model_syn(N,filtlen):
    ...
    model.add(Conv2DTranspose(filters=1, kernel_size=(filtlen,1), strides=(N,1), activation="linear", use_bias=False, kernel_initializer=MDCT_syn_init, input_shape=(None,1,N)))
...
def keras_MDCT_syn(subbands,model):
...
if __name__ == '__main__':
    ...
    subfile=open("mdct_subbands.pickle", 'rb')
    subbands=pickle.load(subfile)
    xrek= keras_MDCT_syn(subbands,model)
