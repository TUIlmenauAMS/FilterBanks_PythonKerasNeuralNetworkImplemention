...
"""
Simple program to use a keras convolutional neural network as implementation of an MDCT synthesis filter bank.
...
def MDCT_syn_init(shape, dtype=None):
    print("Initializing synthesis MDCT weights, shape=", shape)
    N= shape[-1] #Number of subbands
    filtlen=shape[0]
    #weights = keras.backend.placeholder(shape=shape)
...
def generate_model_syn_stereo(N,filtlen):
    ...
    model.add(Conv2DTranspose(filters=1, kernel_size=(filtlen,1), strides=(N,1), activation="linear", use_bias=False, kernel_initializer=MDCT_syn_init, input_shape=(None,2,N) ))
...
def keras_MDCT_syn_stereo(subbands,model):
    ...
    subbands=np.expand_dims(subbands,axis=0) #batch dimension
    #subbands=np.expand_dims(subbands,axis=2) #stereo channels
...
if __name__ == '__main__':
    ...
    model = generate_model_syn_stereo(N,filtlen) 
    subfile=open("mdct_subbands_stereo.pickle", 'rb')
    subbands=pickle.load(subfile)
    xrek= keras_MDCT_syn_stereo(subbands,model)
