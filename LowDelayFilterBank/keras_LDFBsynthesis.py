...
"""
Simple program to use a keras convolutional neural network as implementation of an Low Delay synthesis filter bank.
...
def LDFBsyn_init(shape, dtype=None):
    print("Initializing LDFB synthesis weights, shape=", shape)
    ...
    for k in range(N):
        weights[:,0,0,k]= np.flipud(LDFBproto) * np.sqrt(2.0/N)*np.cos(np.pi/N*(k+0.5)*(np.arange(filtlen)+0.5-N/2)) 
...
def generate_model_syn(N,filtlen):
    ...
    model.add(Conv2DTranspose(filters=1, kernel_size=(filtlen,1), strides=(N,1), activation="linear", use_bias=False, kernel_initializer=LDFBsyn_init, input_shape=(None,1,N)))
...
def keras_LDFB_syn(subbands,model):
...
if __name__ == '__main__':
    ...
    subfile=open("LDFB_subbands.pickle", 'rb')
    subbands=pickle.load(subfile)
    xrek= keras_LDFB_syn(subbands,model)
